...
Always responding in Simplified Chinese.
...
│   ├── Login.tsx
│   ├── Home.tsx              # Dashboard home with module cards
│   ├── SalesRevenue.tsx
...
│   ├── RightsProducts.tsx
│   ├── HomeNetworking.tsx
│   └── sales-revenue/        # Sub-route components per module
│   └── broadband/
│   └── ...
├── components/
│   ├── layout/               # Navbar, Sidebar, ModuleLayout
│   ├── common/               # Shared UI: SubIconNav, TimeRangeSelector, ExportButton, ProtectedRoute
│   ├── charts/               # Chart wrappers and chart components
│   └── kpi/                  # KPI card components
├── hooks/                    # useAuth, useTheme, useModuleData
├── contexts/                 # AuthContext
├── types/                    # Shared TypeScript types
├── utils/                    # Data generators, Excel export, number animations
...
- `AuthContext` (`src/contexts/AuthContext.tsx`) wraps the app and exposes auth state.
- `ProtectedRoute` (`src/components/common/ProtectedRoute.tsx`) redirects unauthenticated users to `/login`.
...
- **ProtectedRoute** (`src/components/common/ProtectedRoute.tsx`) guards authenticated routes.
...
- `src/utils/excelExport.ts` provides typed conversion helpers (`kpiToSheet`, `trendToSheet`, `distributionToSheet`, `regionToSheet`, `topProductsToSheet`, `topRightsToSheet`, `statusStatsToSheet`, `churnRiskToSheet`) and `exportToExcel(filename, sheets)` which generates a multi-sheet `.xlsx` file with auto-sized columns.
